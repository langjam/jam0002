...
def simplify_expr(expr):
  '''Simplify an expression if possible. Not very smart, so mostly constant folding.'''

  if not isinstance(expr, tuple): return expr
  if isinstance(expr, FuncCall): return expr
  if isinstance(expr, Identifier): return expr
